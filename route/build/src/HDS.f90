module HDS
    USE nrtype
    ! physical constants
    USE public_var,           ONLY: RoW, &          ! intrinsic density of water    (kg m-3)
                                    data_step => dt ! simulation time step [sec]
    ...
    subroutine runDepression(NETOPO_in,          & ! input: reach topology
        ...
        type(RCHPRP)        , intent(in)    :: RPARAM_in            ! River reach parameter
        ! real(rkind)         , intent(in)    :: basinPrecip          ! average basin precipitation amount (kg m-2 s-1 = mm s-1)
        real(dp)            ,               :: reachRunoff_local    ! reach runoff (m/s)
        real(dp)            ,               :: reachEvapo_local     ! reach evaporation (m/s)
        real(dp)            ,               :: reachPrecip_local    ! reach precipitation (m/s)
    ...
    subroutine computFlux(iter, pondVol,             & ! input:  iteration index, state variable = pond volume [m3]
        ...
        ! get the forcing
        pInput  = pRate /RoW  ! kg m-2 -> m s-1
        qInput  = qSeas /RoW  + rCoef*pRate/RoW  ! surface runoff ! kg m-2 s-1 -> m s-1
        eLosses = etPond/RoW  ! evaporation losses kg m-2 s-1 -> m s-1
